...
namespace BotDLL
{
    public class DrawStatistic
    {
        ...
        public static DrawStatistic BuildStatistic(LF_ServerInfo info)
        {
            ...
            temp = RemoveTillWord(temp, "labels=", 7);

            string labels = RemoveAfterWord(temp, "&data1=", 0);

            string data1 = RemoveTillWord(temp, "&data1=", 7);
            data1 = RemoveAfterWord(data1, "&data2=", 0);

            string data2 = RemoveTillWord(temp, "&data2=", 7);

            string[] slabels = labels.Split(',');
